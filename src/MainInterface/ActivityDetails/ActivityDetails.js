...
function pad(number, width, filler) {
  ...
  return number.length >= width
    ? number
    : new Array(width - number.length + 1).join(filler) + number;
}
...
export default class ActivityDetails extends Component {
  ...
  valdiateInput = () => {
    ...
    if (nameLength > this.state.validRules.nameMaxLength) {
      errorMsg +=
        "Activity Name Must be within" +
        this.state.validRules.nameMaxLength +
        "characters\n";
    ...
    if (timeLength < this.state.validRules.timeMinLength) {
      errorMsg +=
        "Length of Acvtivity must be greater than " +
        this.state.validRules.timeMinLength +
        "minutes.\n";
      this.setValidOrNot("length", false);
    } else if (timeLength > this.state.validRules.timeMaxLength) {
      errorMsg +=
        "Length of Acvtivity must be smaller than" +
        this.state.validRules.timeMaxLength +
        " Minutes\n";
      this.setValidOrNot("length", false);
    } else {
    ...
    if (this.props.activity !== null) {
      modalContent = (
        <View style={{ height: 300 }}>
          <View
            style={
              this.state.validate.type ? styles.validInput : styles.invalidInput
            }
          >
            <Picker
              selectedValue={this.state.activity.type}
              style={
                this.state.validate.type == false
                  ? styles.invalidInput
                  : styles.validInput
              }
              onValueChange={(val) =>
                this.changeActivityInfoHandler("type", val)
              }
              mode="dropdown"
          ...
          >
            <TextInput
              ...
              onChangeText={(val) =>
                this.changeActivityInfoHandler("activityName", val)
              }
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Text style={styles.textLabel}>
              Estimated Length of Activity in minutes
            </Text>
            <View
              style={
                this.state.validate.length == false
                  ? styles.invalidInput
                  : styles.validInput
              }
            >
              <TextInput
                ...
                value={this.state.activity.length.toString()}
                style={{ width: 90 }}
                onChangeText={(val) =>
                  this.changeActivityInfoHandler("length", val)
                }
                placeholder="50 mins"
              ></TextInput>
            </View>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <Picker
              ...
              onValueChange={(val) =>
                this.changeActivityInfoHandler("energyPtr", val)
              }
              mode="dialog"
              itemStyle={{ fontSize: 17, paddingLeft: width * 0.035 }}
            >
